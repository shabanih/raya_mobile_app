...
import 'sewage_installments_screen.dart';

class SewageChargeScreen extends StatefulWidget {
  const SewageChargeScreen({super.key});

  @override
  State<SewageChargeScreen> createState() =>
      _SewageChargeScreenState();
}

class _SewageChargeScreenState
    extends State<SewageChargeScreen> {
  // =====================================================
  // Colors
  // =====================================================
  ...
  Future<void> loadSewageCharges() async {
    if (mounted) {
      setState(() {
        isLoading = true;
        errorMessage = null;
    ...
    try {
      final result =
      await ApiService().getSewageCharges();

      debugPrint('====================================');
      debugPrint(
        'Sewage CHARGES COUNT: ${result.length}',
      );

      for (final item in result) {
        debugPrint(
          'Sewage CHARGE ITEM: $item',
        );
      }
      ...
      setState(() {
        isLoading = false;
        errorMessage =
        'دریافت هزینه فاضلاب با خطا مواجه شد.';
      });
    }
  }
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'مشاهده و پرداخت هزینه‌های فاضلاب ساختمان',
                  style: TextStyle(
                    fontSize: 11,
                    color: Colors.grey,
                  ),
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'در حال حاضر هیچ هزینه فاضلابی برای شما ثبت نشده است.',
              textAlign:
              TextAlign.center,
              style: TextStyle(
                fontSize: 12,
