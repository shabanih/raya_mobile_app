...
class SewageInstallmentsScreen extends StatefulWidget {
  ...
  const SewageInstallmentsScreen({
    super.key,
    required this.sewageId,
    required this.sewageName,
  });

  @override
  State<SewageInstallmentsScreen> createState() =>
      _SewageInstallmentsScreenState();
}

class _SewageInstallmentsScreenState
    extends State<SewageInstallmentsScreen> {
  bool isLoading = true;
  String? errorMessage;

  Map<String, dynamic>? sewage;
  ...
  Future<void> loadInstallments() async {
    ...
    try {
      final result = await ApiService().getSewageInstallments(
        widget.sewageId,
      );

      debugPrint('==========================================');
      debugPrint('sewage INSTALLMENTS RESULT');
      debugPrint('sewage: ${result['sewage']}');
      ...
      final resultSewage = result['sewage'];
      ...
      setState(() {
        if (resultSewage is Map) {
          sewage = Map<String, dynamic>.from(
            resultSewage,
          );
        }
      ...
      setState(() {
        isLoading = false;
        errorMessage =
        'دریافت اقساط هزینه فاضلاب با خطا مواجه شد.';
      });
    }
  }
...
// =====================================================
  ...
  Widget _buildSewageInfo() {
    final amount =
        sewage?['amount'] ?? 0;

    final installmentCount =
        sewage?['installment_count'] ?? 0;

    final details =
        sewage?['details']?.toString() ?? '';
                ...
                if (installments
                  ...
                  const Center(
                    ...
                    Padding(
                      ...
                      child:
                      Text(
                        'برای این هزینه هنوز قسطی ثبت نشده است.',
                        textAlign:
                        TextAlign.center,
                        style:
                        TextStyle(
